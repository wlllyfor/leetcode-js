...
console.log('111', batchFindAllPath(tree, [1]));
console.log('111', batchFindAllPath(tree, [8, 9]));
...
function findLevels(tree, values) {
  let pathsToValues = [];
  let levels = [[], [], [], []];

  function dfs(node, path) {
    if (values.includes(node.value)) {
      pathsToValues.push([...path, node.value]);
      values = values.filter(val => val !== node.value);
    }

    if (node.children) {
      for (let child of node.children) {
        dfs(child, [...path, node.value]);
      }
    }
  }
  ...
  for (let pathToValue of pathsToValues) {
    let node = {children: tree};

    for (let val of pathToValue) {
      node = node.children.find(child => child.value === val);
      if (!levels[pathToValue.indexOf(val)].includes(val)) {
        levels[pathToValue.indexOf(val)].push(val);
      }
    }
    ...
    function bfs(node, level) {
      ...
      for (let child of node.children) {
        if (!levels[level].includes(child.value)) {
          levels[level].push(child.value);
          bfs(child, level + 1);
        }
      }
...
console.log('222', findLevels(tree, [1]));
console.log('222', findLevels(tree, [8, 9]));
