...
type SubjectForPostType = {
  hub_id: number | null;
  name: string;
  /** 科目は売上科目と同じAPIを使用するため、is_profitはfalse固定 */
  is_profit: false;
};
...
export const useStore = (): {
  postSubject: () => Promise<void>;
    subjectForPost: SubjectForPostType;
    setSubjectForPost: Dispatch<SetStateAction<SubjectForPostType>>;
    isLoading: boolean;
    isStored: boolean;
  ...
  const [ subjectForPost, setSubjectForPost ] = useState<SubjectForPostType>(defaultSubjectForPostType);
  ...
  const postSubject = async (): Promise<void> => {
    ...
    const body = {
      hub_id: subjectForPost.hub_id,
      name: subjectForPost.name,
      is_profit: false,
    };
