...
import { ReactElement, useId, useState, useEffect } from "react";
...
const ModalItem = ({ handleDetailAddition, selectedReceiveStock }: { handleDetailAddition: () => void; selectedReceiveStock: ReceiveStockDbTableType }): ReactElement => {
  ...
  const handleCheckboxChange = () => {
    setIsWithdrawalDetailChecked(!isWithdrawalDetailChecked);
  };
  ...
  return (
    <>
      <ContentAreaWrapper>
        ...
        <div>
          <div className="mb-6">
            <InputCheckbox id={`${inputId}-withdrawal-detail`} checked={isWithdrawalDetailChecked} text="このまま引落詳細を作成する" onChange={handleCheckboxChange} />
          </div>
          {true && (
            <div>
