...
const ModalItemDetailRow = (): ReactElement => {
  const inputId = useId();
  ...
  return (
    <ContentAreaWrapper>
      <FlexWrapperLg>
        ...
        <InputGroup44 id={`${inputId}-contents`} text={"内容"} value={""} isRequired />
        <SelectGroup44 text={"取引先"} options={options4} isMulti={false} />
        <FileUploadIconTextareaGroup
          textareaId={`${inputId}-file`} inputFileUploadIconId={"file"} labelText={"管理メモ"}
          rows={3} value={""}
        />
      </FlexWrapperLg>
    </ContentAreaWrapper>
...
export default ModalItemDetailRow;
